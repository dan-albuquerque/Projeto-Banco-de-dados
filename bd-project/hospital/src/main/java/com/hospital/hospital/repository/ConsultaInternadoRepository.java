...
public class ConsultaInternadoRepository {
        ...
        public List<ConsultaInternado> selectConsultaInternadoByPaciente(String cpf) {
                return jdbcTemplate.query("SELECT * FROM consulta_internado WHERE fk_paciente_cpf = ?",
                                consultaInternadoMapper,
                                cpf);
        }

        public List<ConsultaInternado> selectConsultaInternadoByMedico(String cpf) {
                return jdbcTemplate.query("SELECT * FROM consulta_internado WHERE fk_medico_cpf = ?",
                                consultaInternadoMapper,
                                cpf);
        }
        ...
        @SuppressWarnings("deprecation")
        public int addInternedConsultation(String pacienteCpf, String medicoCpf, String conduta, String evolucao,
                        String historicoExames, String dataConsulta) {
                String sql = "CALL AddInternedConsultation(?, ?, ?, ?, ?, ?)";
                return jdbcTemplate.queryForObject(sql, new Object[] { pacienteCpf, medicoCpf, conduta, evolucao,
                                historicoExames, dataConsulta }, Integer.class);
        }
}
