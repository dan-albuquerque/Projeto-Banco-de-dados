...
public class ConsultaUrgenciaRepository {
    ...
    public void insertConsultaUrgencia(ConsultaUrgencia consultaUrgencia) {
        jdbcTemplate.update(
                "insert into  consulta_urgencia (fk_medico_cpf, fk_paciente_urgencia_cpf, data_realizacao, fk_registro_urgencia_codigo)  values  (?, ?, ?, ?);",
    ...
    public ConsultaUrgencia selectConsultaUrgencia(int fk_registro_urgencia_codigo,
        ...
        return jdbcTemplate.queryForObject(
                "SELECT * FROM consulta_urgencia WHERE fk_registro_urgencia_codigo = ? AND fk_medico_cpf = ? AND fk_paciente_urgencia_cpf = ?",
    ...
    public void deleteConsultaUrgencia(int fk_registro_urgencia_codigo,
            ...
            String fk_paciente_urgencia_cpf) {
        jdbcTemplate.update(
                "DELETE FROM consulta_urgencia WHERE fk_registro_urgencia_codigo = ? AND fk_medico_cpf = ? AND fk_paciente_urgencia_cpf = ?",
                fk_registro_urgencia_codigo,
                fk_medico_cpf,
                fk_paciente_urgencia_cpf);
    }
    ...
    public List<ConsultaUrgencia> selectConsultaUrgenciaByPaciente(String cpf) {
        return jdbcTemplate.query("SELECT * FROM consulta_urgencia WHERE fk_paciente_urgencia_cpf = ?",
    ...
    public List<ConsultaUrgenciaDTO> selectConsultaUrgenciaDTO() {
        return jdbcTemplate.query(
                "SELECT c.data_realizacao, p.nome as paciente_nome, m.nome as medico_nome FROM consulta_urgencia c " +
                        "INNER JOIN paciente p ON c.fk_paciente_urgencia_cpf = p.cpf " +
                        "INNER JOIN medico m ON c.fk_medico_cpf = m.cpf",
                consultaUrgenciaDTOMapper
        );
    }

    public ConsultaUrgenciaDTO selectConsultaUrgenciaDTOById(int fk_registro_urgencia_codigo, String fk_medico_cpf, String fk_paciente_urgencia_cpf) {
        return jdbcTemplate.queryForObject(
                "SELECT c.data_realizacao, p.nome as paciente_nome, m.nome as medico_nome FROM consulta_urgencia c " +
                        "INNER JOIN paciente p ON c.fk_paciente_urgencia_cpf = p.cpf " +
                        "INNER JOIN medico m ON c.fk_medico_cpf = m.cpf " +
                        "WHERE c.fk_registro_urgencia_codigo = ? AND c.fk_medico_cpf = ? AND c.fk_paciente_urgencia_cpf = ?",
                consultaUrgenciaDTOMapper,
                fk_registro_urgencia_codigo, fk_medico_cpf, fk_paciente_urgencia_cpf
        );
    }
}
