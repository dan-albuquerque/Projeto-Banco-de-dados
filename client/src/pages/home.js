...
export const getServerSideProps = async (context) => {
  ...
  try {
    const [
      latestConsultationsResponse,
      doctorsCountResponse,
      internsCountResponse,
      doctorWithMoreConsultationsResponse,
      internStarResponse,
      patientsMoreUrgentResponse,
      internedCountResponse,
      urgentCountResponse,
      specialityDemandResponse,
      avgConsultasResponse
    ] = await Promise.all([
...
export default function Home({
  ...
  // Acessando o primeiro item do array avgConsultas
  const mediaConsultas = avgConsultas.length > 0 ? avgConsultas[0].media_consultas : 0;
  const anoConsultas = avgConsultas.length > 0 ? avgConsultas[0].ano : 'N/A';
      ...
      <div className='items-center flex flex-col'>
        ...
        <div className='px-6 w-[90%] h-42 rounded-lg bg-gradient-to-r from-blue-200 to-blue-500 flex justify-between mt-6 shadow-lg items-center'>
          <div className='flex flex-col gap-1 justify-center'>
            ...
            <p className='italic text-grey-100'>No ano de {anoConsultas}, foram efetivadas {mediaConsultas} Consultas!</p>
          </div>
